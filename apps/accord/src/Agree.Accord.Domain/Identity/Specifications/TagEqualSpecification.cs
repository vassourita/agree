...
{
    public TagEqualSpecification(DiscriminatorTag tag) => Expression = u => u.Tag == tag;

    public TagEqualSpecification(ApplicationUser account) => Expression = u => u.Tag == account.Tag;
}